...
void jy62MessageRecord(void)
{
    int idx = 0; // 接受数据位指针
    int i = 0;
    while (idx < JY62_MESSAGE_LENGTH)
    { // 在判断JY62_MESSAGE_LENGTH(200)的长度内进行查找
        while (jy62Receive[idx] != 0x55)
            idx++;       // 接收数据为指针找到包头0x55
        uint8_t sum = 0; // 计算校验位
        for (i = 0; i < 10; i++)
            sum += jy62Receive[idx + i]; // 判断校验位正确与否
        if (sum == jy62Receive[idx + 10])
        { // 校验位正确，认为收到正确包
            for (i = 0; i < 11; i++)
                jy62Message[i] = jy62Receive[idx + i]; // 读入jy62Message中
            Decode();                                  // 解析
            idx += 11;                                 // 继续下一个包读入
        }
        else
